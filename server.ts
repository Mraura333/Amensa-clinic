...
async function startServer() {
  ...
  // Store pending booking details in memory to handle background webhook/redirect resolution
  const pendingCashfreeBookings = new Map<string, any>();

  // Reusable helper function to instantiate confirmed bookings upon successful payment
  async function instantiatePaidBooking(bookingDetails: any, paymentId: string, provider: string) {
    const db = getDB();
    const { 
      patientId, 
      patientName, 
    ...
    if (!patientName || !mobile || !bookingType || !preferredDate || !preferredTimeSlot) {
      throw new Error("Required patient scheduler fields are missing.");
    }
    ...
    for (const item of items) {
      ...
      // Check if this specific appointment already exists to prevent duplicate transactions
      const existingAppt = db.appointments.find(a => 
        a.patientId === resolvedPatientId && 
        a.selectedItemId === item.id && 
        a.preferredDate === preferredDate && 
        a.preferredTimeSlot === preferredTimeSlot &&
        a.notes?.includes(paymentId)
      );
      if (existingAppt) {
        bookingsCreated.push(existingAppt);
        continue;
      }
      ...
      const newBooking: Appointment = {
        ...
        locationId: !isHome ? locationId : undefined,
        pricePaid: item.price,
        status: "Paid", // Set strictly to "Paid" status as per requirement
        notes: notes ? `${notes} (Paid via ${provider}: ${paymentId})` : `Paid via ${provider}: ${paymentId}`,
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString()
      };
    ...
    if (bookingsCreated.length > 0) {
      // Add the visit charge to the price paid of the first booking (only if it wasn't already added)
      if (Number(visitCharge) > 0) {
        const firstBooking = bookingsCreated[0];
        // If we just newly created it, add visit charge
        if (!db.appointments.some(a => a.id === firstBooking.id)) {
          firstBooking.pricePaid += Number(visitCharge);
        }
      }

      // Add newly created to database
      bookingsCreated.forEach(booking => {
        if (!db.appointments.some(a => a.id === booking.id)) {
          db.appointments.unshift(booking);
        }
      });
      ...
      addNotification(
        resolvedPatientId,
        `Online Payment Successful (${provider})`,
        `Congratulations! Your payment of ₹${totalAmount} has been processed successfully. We have confirmed your appointments for: ${items.map((i: any) => i.name).join(', ')}.`
      );
  ...
  app.post("/api/cashfree/create-order", async (req, res) => {
    const { amount, patientName, patientPhone, patientEmail, bookingDetails } = req.body;
    ...
    const isSimulated = !appId || !secretKey;

    const orderId = `cf_${Date.now()}_${Math.floor(1000 + Math.random() * 9000)}`;

    if (bookingDetails) {
      pendingCashfreeBookings.set(orderId, bookingDetails);
    }

    if (isSimulated) {
      const simulatedSessionId = `session_sim_${Math.random().toString(36).substring(2, 11)}`;
      res.json({
        orderId,
        paymentSessionId: simulatedSessionId,
        isSimulated: true,
        environment: process.env.CASHFREE_ENV || 'sandbox',
        message: "Running in Cashfree simulation mode. Configure CASHFREE_APP_ID and CASHFREE_SECRET_KEY to connect your live gateway."
      });
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        order_currency: "INR",
        customer_details: {
          customer_id: `cust_${Math.floor(100000 + Math.random() * 900000)}`,
          customer_phone: patientPhone.length === 10 ? `+91${patientPhone}` : patientPhone,
          customer_name: patientName || "Guest User",
          customer_email: patientEmail || `${patientPhone || 'guest'}@amensadiagnostics.com`
        },
        order_meta: {
          return_url: `${process.env.APP_URL || 'http://localhost:3000'}/?cf_order_id={order_id}`,
      ...
      if (cfResponse.ok) {
        ...
        res.status(cfResponse.status).json({
          error: "Failed to create Cashfree checkout order.",
          details: data
        });
      }
    } catch (err: any) {
      console.error("Cashfree order creation exception:", err);
      res.status(500).json({ error: "Failed to create Cashfree order due to exception on server side.", details: err.message });
    }
  });

  // Verify Cashfree Payment Signature & Instantiate Booking
  app.post("/api/cashfree/verify-payment", async (req, res) => {
    const { orderId, isSimulated, bookingDetails } = req.body;

    if (!orderId) {
      res.status(400).json({ error: "Missing Cashfree orderId parameter." });
      return;
    }

    // Resolve booking details from body or from our pending background memory map
    let resolvedDetails = bookingDetails;
    if (!resolvedDetails) {
      resolvedDetails = pendingCashfreeBookings.get(orderId);
    }

    if (!resolvedDetails) {
      res.status(400).json({ error: "Booking details not found for this transaction session. Please contact clinic support." });
      return;
    }

    const appId = process.env.CASHFREE_APP_ID;
    const secretKey = process.env.CASHFREE_SECRET_KEY;
    const isMock = isSimulated || !appId || !secretKey || orderId.startsWith("cf_order_sim_") || orderId.includes("_sim_");

    if (isMock) {
      try {
        const bookings = await instantiatePaidBooking(resolvedDetails, orderId, "Cashfree (Simulated)");
        pendingCashfreeBookings.delete(orderId);
        res.json({
          success: true,
        ...
        res.status(500).json({ error: "Failed to instantiate booking for simulated payment.", details: err.message });
      }
      return;
    }

    // Live Pull Verification from Cashfree Gateway
    try {
      const isProd = process.env.CASHFREE_ENV === "production";
      const cfUrl = isProd 
      ...
      const cfResponse = await fetch(cfUrl, { method: "GET", headers });
      const orderData = await cfResponse.json();

      if (!cfResponse.ok) {
        console.error("Cashfree verification fetch error:", orderData);
        res.status(cfResponse.status).json({ error: "Failed to retrieve order status from Cashfree.", details: orderData });
        return;
      }

      if (orderData.order_status === "PAID") {
        const bookings = await instantiatePaidBooking(resolvedDetails, orderId, "Cashfree");
        pendingCashfreeBookings.delete(orderId);
        res.json({
          success: true,
        ...
        res.status(400).json({ 
          error: "Payment not completed yet on Cashfree.", 
          status: orderData.order_status,
          message: "Your payment transaction is in state: " + orderData.order_status
        });
      }
    } catch (err: any) {
      console.error("Cashfree verification exception:", err);
      res.status(500).json({ error: "Failed to verify Cashfree payment.", details: err.message });
    }
  });

  // Cashfree Webhook Endpoint
  app.post("/api/cashfree/webhook", async (req, res) => {
    // Send immediate 200 OK status to Cashfree (compliance requirement)
    res.status(200).send("OK");

    const payload = req.body || {};
    console.log("Cashfree webhook event received:", JSON.stringify(payload));

    const orderId = payload.data?.order?.order_id || payload.order?.order_id || payload.orderId;

    if (!orderId) {
      console.warn("Cashfree Webhook missing order ID context.");
      return;
    }

    const bookingDetails = pendingCashfreeBookings.get(orderId);
    if (!bookingDetails) {
      console.log(`No pending booking details found in memory for Cashfree order ${orderId}. Already processed.`);
      return;
    }

    const appId = process.env.CASHFREE_APP_ID;
    const secretKey = process.env.CASHFREE_SECRET_KEY;
    const isMock = orderId.startsWith("cf_order_sim_") || !appId || !secretKey;

    if (isMock) {
      try {
        await instantiatePaidBooking(bookingDetails, orderId, "Cashfree Webhook (Simulated)");
        pendingCashfreeBookings.delete(orderId);
        console.log(`Successfully completed simulated booking via Webhook for order ${orderId}`);
      } catch (err) {
        console.error("Failed to process simulated webhook:", err);
      }
      return;
    }

    // 1. Verify Cryptographic Signature from Cashfree headers
    const signature = req.headers["x-webhook-signature"];
    const timestamp = req.headers["x-webhook-timestamp"];

    let signatureVerified = false;
    if (signature && timestamp && secretKey && (req as any).rawBody) {
      try {
        const rawBodyStr = (req as any).rawBody.toString("utf-8");
        const signatureData = timestamp + rawBodyStr;
        ...
        if (computedSignature === signature) {
          ...
          console.log(`[Cashfree Webhook] Cryptographic signature verified successfully for order ${orderId}.`);
        } else {
          console.warn(`[Cashfree Webhook] Cryptographic signature mismatch for order ${orderId}.`);
        }
      } catch (err: any) {
        console.error("[Cashfree Webhook] Cryptographic verification failed with exception:", err);
      }
    } else {
      console.warn(`[Cashfree Webhook] Cryptographic headers/rawBody missing for order ${orderId}. Relying on secure Active Pull verification.`);
    }

    // 2. Active Pull Verification standard for webhooks (prevents webhook signature forgery/tampering)
    try {
      const isProd = process.env.CASHFREE_ENV === "production";
      const cfUrl = isProd 
        ? `https://api.cashfree.com/pg/orders/${orderId}` 
      ...
      if (cfResponse.ok && orderData.order_status === "PAID") {
        await instantiatePaidBooking(bookingDetails, orderId, signatureVerified ? "Cashfree Webhook (Verified)" : "Cashfree Webhook (Active Pull)");
        pendingCashfreeBookings.delete(orderId);
        console.log(`Successfully completed paid booking via secure Webhook Active Pull Verification for order ${orderId}`);
      } else {
        console.log(`Cashfree order status is ${orderData?.order_status || 'unknown'}. Skipping booking resolution.`);
      }
    } catch (err) {
      console.error("Exception in secure webhook active pull verification:", err);
    }
  });
