...
export default function App() {
  ...
  // Cashfree Redirect Payment States
  const [isVerifyingPayment, setIsVerifyingPayment] = useState(false);
  const [verificationOrderId, setVerificationOrderId] = useState<string | null>(null);
  const [paymentVerifySuccess, setPaymentVerifySuccess] = useState(false);
  ...
  // Cashfree redirection detector
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const cfOrderId = params.get('cf_order_id');
    if (cfOrderId) {
      setVerificationOrderId(cfOrderId);
      setIsVerifyingPayment(true);
      
      // Clear query params to make sure refresh/bookmark doesn't re-trigger verification
      ...
      fetch('/api/cashfree/verify-payment', {
        ...
        .then(res => {
          if (!res.ok) {
            return res.json().then(errData => {
              throw new Error(errData.error || 'Failed to verify online payment.');
            });
          }
          return res.json();
        })
        .then(data => {
          if (data.success) {
            setPaymentVerifySuccess(true);
            setVerifiedBookings(data.bookings || []);
            // Clean up cart
            setCartItems([]);
          } else {
            setPaymentVerifyError('Payment verification was not successful.');
          }
        })
        .catch(err => {
          console.error("Cashfree redirect verification failed:", err);
          setPaymentVerifyError(err.message || 'Payment verification encountered a network error.');
        })
        .finally(() => {
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#F8FBFF] font-sans antialiased text-[#1A1A1A]">
        ...
        {/* Full Screen Live Cashfree Redirect Verification Status Overlay */}
        <AnimatePresence>
          {(isVerifyingPayment || verificationOrderId) && (
            <div className="fixed inset-0 z-[11000] flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-md">
              <motion.div
                initial={{ opacity: 0, scale: 0.95, y: 40 }}
                animate={{ opacity: 1, scale: 1, y: 0 }}
                exit={{ opacity: 0, scale: 0.95, y: 40 }}
                className="bg-white rounded-[40px] border border-slate-100 shadow-2xl p-8 max-w-lg w-full text-center space-y-6 font-sans"
              >
                {/* Loader Section */}
                {isVerifyingPayment && (
                  <div className="space-y-6 py-8">
                    <Loader2 className="w-16 h-16 text-[#0066CC] animate-spin mx-auto" />
                    <div className="space-y-2">
                      <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight">Verifying Secure Payment</h3>
                      <p className="text-xs text-slate-400 max-w-xs mx-auto leading-relaxed">
                        Querying Cashfree merchant vaults to securely verify transaction signatures and establish clinical records...
                      </p>
                    </div>
                  </div>
                )}
                ...
                {/* Success Screen */}
                {!isVerifyingPayment && paymentVerifySuccess && (
                  <div className="space-y-6 py-4">
                    <div className="w-24 h-24 bg-emerald-50 text-emerald-600 border border-emerald-100 rounded-full flex items-center justify-center mx-auto text-5xl shadow-md">
                      <Check className="w-12 h-12 stroke-[3]" />
                    </div>
                    <div className="space-y-2">
                      <h3 className="text-2xl font-black text-slate-900 tracking-tight">Online Payment Confirmed!</h3>
                      <p className="text-xs text-slate-400 max-w-sm mx-auto leading-normal">
                        Congratulations! Your online payment was validated and your appointment has been updated to <span className="text-emerald-600 font-extrabold">"Paid"</span> status.
                      </p>
                    </div>

                    <div className="bg-slate-50 border border-slate-200/60 rounded-3xl p-5 text-left divide-y divide-slate-100 max-w-md mx-auto">
                      <div className="pb-2.5 flex justify-between items-center text-xs">
                        <span className="text-slate-400 font-bold uppercase text-[9px]">Order Reference</span>
                        <span className="font-mono font-black text-[#0066CC] text-right truncate max-w-[200px]">{verificationOrderId}</span>
                      </div>
                      <div className="py-2.5 flex justify-between items-center text-xs">
                        <span className="text-slate-400 font-bold uppercase text-[9px]">Booking Codes</span>
                        <span className="font-mono font-black text-slate-800 text-right">
                          {verifiedBookings.map(b => b.id).join(', ') || 'Confirmed'}
                        </span>
                      </div>
                      <div className="pt-2.5 flex justify-between items-center text-xs">
                        <span className="text-slate-400 font-bold uppercase text-[9px]">Portal Authorization</span>
                        <span className="font-bold text-slate-800">Auto-created Profile</span>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-3 justify-center max-w-sm mx-auto">
                      <button
                        ...
                        className="w-1/2 py-3.5 border border-slate-200 hover:bg-slate-50 text-slate-700 font-black text-xs rounded-xl transition-all cursor-pointer shadow-sm"
                      >
                        🖨️ Print Receipt
                      </button>
                      <button
                        onClick={() => {
                          ...
                          setPaymentVerifySuccess(false);
                          setIsPatientPortalOpen(true);
                        }}
                        className="w-1/2 py-3.5 bg-[#0066CC] hover:bg-[#0052CC] text-white font-black text-xs rounded-xl transition-all cursor-pointer shadow-md shadow-blue-100"
                      >
                        Patient Dashboard
                      </button>
                    </div>
                ...
                {/* Failure Screen */}
                {!isVerifyingPayment && paymentVerifyError && (
                  <div className="space-y-6 py-4">
                    <div className="w-24 h-24 bg-rose-50 text-rose-600 border border-rose-100 rounded-full flex items-center justify-center mx-auto text-5xl shadow-md">
                      <AlertTriangle className="w-12 h-12" />
                    </div>
                    <div className="space-y-2">
                      <h3 className="text-2xl font-black text-slate-900 tracking-tight">Verification Incomplete</h3>
                      <p className="text-xs text-rose-700 font-bold max-w-sm mx-auto">
                        We could not verify your Cashfree order status. Please contact Amensa support if you have already been charged.
                      </p>
                      <p className="text-[10px] text-slate-400 bg-rose-50/50 p-3 rounded-xl border border-rose-100 font-mono mt-2">
                    ...
                    <div className="flex gap-2 justify-center max-w-xs mx-auto">
                      <button
                        ...
                        className="w-full py-3.5 bg-slate-900 hover:bg-slate-800 text-white font-black text-xs rounded-xl transition-all cursor-pointer shadow-md"
                      >
                        Back to Cart & Retry
                      </button>
                    </div>
